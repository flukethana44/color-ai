...
function buildPrompt(hex, department, subject) { const { r, g, b } = hexToRgb(hex); return `Color: ${colorNameEN(hex)}\nHEX: ${hex.toUpperCase()}  RGB: ${r} / ${g} / ${b}\n\nDepartment: ${department.name}\nSubject: ${subject.label}\n\nStyle: ${department.style.join(', ')}\nShapes: ${department.shapes.join(', ')}\nObjects: ${department.objects.slice(0, 4).join(', ')}`; }

function ImageCard({ hex, department, subject, index }) {
  const [image, setImage] = useState(null), [error, setError] = useState(null), [open, setOpen] = useState(false);
  const request = async () => { setError(null); setImage(null); try { const response = await fetch('/api/generate-image', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ hex, department: department.name, subject: subject.label, style: department.style, shapes: department.shapes, objects: department.objects }) }); const data = await response.json(); if (!response.ok || !data.image) throw new Error(data.error || 'เกิดข้อผิดพลาด'); setImage(data.image); } catch (err) { setError(err.message); } };
  useEffect(() => { request(); }, [hex, department.id, subject.label]);
  ...
  return <article className="gcard"><div className="gcard-visual">{image ? <img className="gcard-img" src={image} alt={`${subject.label} concept`} /> : error ? <div className="img-status img-error"><p>สร้างภาพไม่สำเร็จ: {error}</p><button type="button" className="img-retry-btn" onClick={request}>ลองใหม่</button></div> : <div className="img-status img-loading"><div className="loading-dot-row"><span className="loading-dot" /><span className="loading-dot" /><span className="loading-dot" /></div></div>}</div><div className="gcard-body"><p className="gcard-type">{department.name}</p><p className="gcard-title">{subject.label}</p><p className="gcard-desc">แนวคิดงาน {subject.label} ที่ใช้สี{colorNameTH(hex)}เป็นสีนำ</p><div className="gcard-usage">{usage}</div><button className="gcard-prompt-toggle" onClick={() => setOpen(!open)}>{open ? 'ซ่อน image prompt' : 'ดู image prompt >'}</button>{open && <pre className="gcard-prompt show">{buildPrompt(hex, department, subject)}</pre>}</div></article>;
}
...
export default function GuideResults({ guide, department }) {
  ...
  const hex = guide.color, colorName = colorNameTH(hex), { r, g, b } = hexToRgb(hex), { tone, mood } = toneAndMood(hex);
